        ...
        private void ImplicitExpression()
        {
            ...
            }))
            {
                do
                {
                    ...
                    {
                        bool __isAwait = CurrentSymbol != null && String.Equals (CurrentSymbol.Content, "await", StringComparison.Ordinal); AcceptAndMoveNext (); if (__isAwait) { while (At (CSharpSymbolType.WhiteSpace)) { AcceptAndMoveNext (); } if (AtIdentifier (allowKeywords: true)) { AcceptAndMoveNext (); } }
                    }
                }
