...
// RunAndWaitForNavigationAsync is [Obsolete] in Playwright (CS0612), and its modern replacements -
// WaitForURLAsync / RunAndWaitForURLAsync - are the wrong tool here: a WebForms postback navigates
// back to the SAME URL it started on, and WaitForURL returns immediately when the current URL already
// matches, so it would race the postback instead of waiting for it to complete. "Wait for the next
// navigation" is exactly what these tests need, so the obsolete API stays and the warning is silenced.
