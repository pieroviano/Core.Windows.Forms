...
$pairs = @(
    ...
    #
    # Fixes: literal replacements applied after retargeting, each on one line so the page's line numbers
    # still match mono/data in a stack trace. A fix that matches nothing fails the generator.
    #
    #   ?op=Add with no &bnd= - the form ASP.NET's own help page links to, and what anyone types - threw a
    #   NullReferenceException: BuildOperationInfo calls FindPort (null, null), and a null protocol matches
    #   no port. Without a binding, describe the operation through the SOAP port, as ASP.NET does.
...
foreach ($p in $pairs) {
    ...
    foreach ($fix in @($p.Fixes | Where-Object { $_ })) {
        if (-not $text.Contains($fix[0])) { throw "gen-config: fix for $($p.In) matched nothing: $($fix[0])" }
