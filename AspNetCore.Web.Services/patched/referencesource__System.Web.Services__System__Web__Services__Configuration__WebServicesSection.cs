    ...
    public sealed class WebServicesSection : ConfigurationSection {
                        ...
                        if (this.protocolImporterTypes.Length == 0) {
                            ...
                            List<Type> protocolImporterList = new List<Type>();
                                ...
                                if ((enabledProtocols & WebServiceProtocols.HttpSoap) != 0) {
                                    /* SoapProtocolImporter: WSDL import needs serialization codegen, absent on .NET Core */
                                }
                            if ((enabledProtocols & WebServiceProtocols.HttpSoap12) != 0) {
                                /* Soap12ProtocolImporter: WSDL import needs serialization codegen, absent on .NET Core */
                            }
                            if ((enabledProtocols & WebServiceProtocols.HttpGet) != 0) {
                                /* HttpGetProtocolImporter: WSDL import, absent on .NET Core */
                            }
                            if ((enabledProtocols & WebServiceProtocols.HttpPost) != 0) {
                                /* HttpPostProtocolImporter: WSDL import, absent on .NET Core */
                            }
        ...
        internal Type[] SoapTransportImporters {
            get {
                ...
                retval = new Type [SoapTransportImporterTypes.Count]; // SoapHttpTransportImporter: WSDL import, absent
        ...
        Type[] mimeImporterTypes = new Type[0]; // MIME importers: WSDL import, absent on .NET Core
