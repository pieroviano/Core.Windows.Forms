...
namespace System.Web.Helpers
{
    internal static class CryptoUtil
    {
        ...
        private static Func<SHA256> GetSHA256Factory()
        {
            ...
            try
            {
                using (SHA256 sha256 = SHA256.Create ()) // port: SHA256Cng is Windows-only
                {
                    return () => SHA256.Create (); // port: SHA256Cng is Windows-only
                }
            }
            ...
            return () => SHA256.Create (); // port: CAPI provider is Windows-only
        }
    }
}
